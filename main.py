...
def main_worker(args):
    ...
    # Set up directories
    run_base_dir, ckpt_base_dir, log_base_dir, log_base_dir_rewind = get_directories(args)
    ...
    writer = SummaryWriter(log_dir=log_base_dir_rewind)
    epoch_time = AverageMeter("epoch_time", ":.4f", write_avg=False)
    validation_time = AverageMeter("validation_time", ":.4f", write_avg=False)
    train_time = AverageMeter("train_time", ":.4f", write_avg=False)
    progress_overall = ProgressMeter(
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_best or save or epoch == args.epochs - 1:
            ...
            if args.conv_type == "LCSConv":
                save_checkpoint(
                    ...
                    is_best,
                    filename=ckpt_base_dir / f"epoch_rewind_{epoch}.state",
                    save=save,
                )
        ...
        progress_overall.display(epoch)
        progress_overall.write_to_tensorboard(
            writer, prefix="diagnostics_rewind", global_step=epoch
        )

        writer.add_scalar("test_rewind/lr", cur_lr, epoch)
        end_epoch = time.time()
        ...
        if args.conv_type == "LCSConv":
            ...
            for n, m in model.named_modules():
                if isinstance(m, LCSConv):
                    ...
                    count += total_params
            total_sparsity = 100 - (100 * sum_sparse / count)
            writer.add_scalar("sparsity_rewind/total", total_sparsity, epoch)
        end_epoch = time.time()


    write_result_to_csv(
        best_acc1=best_acc1,
        best_acc1_bin=best_acc1_bin,
        best_train_acc1=best_train_acc1,
    ...
    if args.conv_type == "LCSConv":
        ...
        if not os.path.exists("runs/layerwise_sparsity_rewind"):
            os.mkdir("runs/layerwise_sparsity_rewind")
        with open("runs/layerwise_sparsity_rewind/{}.json".format(args.name), "w") as f:
...
def _run_dir_exists(run_base_dir):
    ...
    log_base_dir_rewind = run_base_dir / "logs_rewind"
    ckpt_base_dir = run_base_dir / "checkpoints"

    return log_base_dir.exists() or log_base_dir_rewind.exists() or ckpt_base_dir.exists()
...
def get_directories(args):
    ...
    return run_base_dir, ckpt_base_dir, log_base_dir, log_base_dir_rewind
